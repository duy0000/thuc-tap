...
const createScheduleValidationSchema = () => {
  ...
  for (let i = 1; i <= 12; i++) {
    validationFields[`on${i}`] = Yup.string()
      .nullable()
      .when('number_schedule', (number_schedule, schema) => {
        // console.log('validation number_schedule: ', number_schedule, i)
        if (number_schedule[0] >= i) {
          return schema.required(`Vui lòng nhập thời gian bật ${i}!`)
        }
        return schema
      })
    ...
    validationFields[`off${i}`] = Yup.string()
      ...
      .when('number_schedule', (number_schedule, schema) => {
        // console.log('validation number_schedule: ', number_schedule, i)
        if (number_schedule[0] >= i) {
          return schema.required(`Vui lòng nhập thời gian tắt ${i}!`)
        }
        return schema
      })
      .test(
...
export const schemaValidationSetupScheduleBMS = createScheduleValidationSchema()
