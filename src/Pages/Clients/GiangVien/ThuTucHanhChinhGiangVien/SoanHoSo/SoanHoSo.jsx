...
function SoanHoSo() {
  ...
  const handleSubmitForm = async (e) => {
    ...
    const newDataHoSoYeuCau = {
      ...
      MC_TTHC_GV_GuiYeuCau_TrangThai_GhiChu: 'Chưa tiếp nhận',
    ...
    let idGuiYeuCau
    let countTPHSKemTheoBatBuoc = 0
    dataChiTietThuTuc?.ThanhPhanHoSo?.forEach((item) => {
      if (item?.MC_TTHC_GV_ThanhPhanHoSo_BatBuoc === true) {
        return countTPHSKemTheoBatBuoc++
      }
    })
    try {
      ...
      if (listThanhPhanHoSoFiles?.length < countTPHSKemTheoBatBuoc) {
        Swal.fire({
          icon: 'error',
          title: 'Lỗi',
          text: 'Thiếu dữ liệu!',
          footer: 'Vui lòng chọn đầy đủ giấy tờ kèm theo bắt buộc!',
        })
        return
      } else {
