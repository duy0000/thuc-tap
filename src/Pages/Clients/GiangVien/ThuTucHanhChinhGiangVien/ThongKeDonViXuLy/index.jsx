import { getTTHCGVThongKeTheoDVXuLy } from '@/Apis/ApiGiangVien/ThuTucHanhChinhGiangVien/apiThongKeTTHCGV'
...
function TTHCGVThongKeDonViXuLy({ dataFilterCommon }) {
  ...
  const { data: listThongKeDVXuLy, isLoading } = useQuery({
    queryKey: [
      TTHCGV_QUERY_KEY.SP_MC_TTHC_GV_Load_ThongKe_TheoDonViXuLy,
      dataFormFilter,
      dataFilterCommon,
    ],
    queryFn: async () => {
      const response = await getTTHCGVThongKeTheoDVXuLy({
        NgayBatDau: !isEmpty(dataFormFilter.NgayBatDau)
          ? dataFormFilter.NgayBatDau
          : dataFilterCommon.dieuKienLoc === DATA_AUDIT_KEY[2].value ||
  ...
  const handleExportExcel = async () => {
    ...
    worksheet.getCell('B1').value = 'ĐƠN VỊ XỬ LÝ'
    ...
    saveAs(blob, `TTHCGV_ThongKeDonViXuLy_${currentDate}.xlsx`)
  }

  return (
    <div className="bg-white p-2 rounded-lg shadow-md">
      <h3 className="text-center font-bold uppercase text-lg md:text-xl mb-2">
        Thống Kê Theo <span className="text-red-600">Đơn vị xử lý</span>
      </h3>
          ...
          <thead className="sticky top-0 z-10 text-xs text-white bg-sky-800 dark:bg-gray-700 dark:text-gray-400 shadow rounded-md">
            <tr className="border-b border-white">
              ...
              <th
                scope="col"
                rowSpan={2}
                className="px-6 py-1 text-center border-r border-white uppercase w-48"
              >
                Đơn vị xử lý
              </th>
              <th
                scope="col"
                rowSpan={2}
                className="px-6 py-1 text-center border-r border-white uppercase w-20"
              >
                Tổng số đề nghị
              </th>
              <th
...
export default TTHCGVThongKeDonViXuLy
