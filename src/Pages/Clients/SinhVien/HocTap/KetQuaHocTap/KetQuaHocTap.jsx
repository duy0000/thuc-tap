    ...
    <>
      ...
      >
        ...
        >
          ...
          <div>
            <span className="font-medium">Ghi chú:</span> Sinh viên click vào
            <span className="font-medium"> tên môn học</span> để xem "Chi tiết
            kết quả học tập" của từng môn học.
          </div>
        </div>
                      ...
                      <tbody>
                        {listMonHoc.map((mh, i) => {
                          if (mh.TC_SV_KetQuaHocTap_HocKy === hk) {
                            return (
                              <tr key={i}>
                                <td className="transition-all sticky left-0 top-0 bg-white font-semibold duration-200 hover:text-uneti-primary cursor-pointer py-2 px-4 text-uneti-primary-lighter border-solid border-uneti-primary border-opacity-30 border-b-0 border-l-0 border-r-0">
                                  <Link
                                    to={`ket-qua-hoc-tap-chi-tiet/${mh.TC_SV_KetQuaHocTap_MaLopHocPhan}`}
                                  >
                                    <Tooltip title="Click để xem chi tiết">
                                      <p>{mh.TC_SV_KetQuaHocTap_TenMonHoc}</p>
                                    </Tooltip>
                                  </Link>
                                  <span className="absolute w-[1px] h-[100%] top-0 right-[-0.5px] bg-uneti-primary opacity-30"></span>
                                </td>
