...
import { THONG_KE_LIST } from './constants'
...
export default function CSDLDonVi() {
  ...
  const handleSelectedItemsChange = (event, ids) => {
    // TODO: Cập nhật cơ sở dữ liệu đơn vị cho từng khoa
    setSelectedItems(ids)
  }

  const groupNganh = useCallback(() => {
    return nganh.reduce((acc, item) => {
      const { TenDonVi, TenHeDaoTao, TenKhoa } = item
      if (acc[TenDonVi]) {
        if (acc[TenDonVi][TenKhoa]) {
          if (acc[TenDonVi][TenKhoa][TenHeDaoTao]) {
            acc[TenDonVi][TenKhoa][TenHeDaoTao].children.push(item)
          } else {
            acc[TenDonVi][TenKhoa][TenHeDaoTao] = {
              ID: uniqueId(),
              children: [item],
            }
          }
        } else {
          acc[TenDonVi][TenKhoa] = {}
          acc[TenDonVi][TenKhoa][TenHeDaoTao] = {
            ID: uniqueId(),
            children: [item],
          }
  ...
  useEffect(() => {
    // group by TenDonVi, TenHeDaoTao, TenKhoa
    setTreeData(groupNganh())
  }, [nganh])
          ...
          <div className="flex flex-col mt-4 gap-4">
            ...
            <div className="flex gap-6">
              <div className="flex-1 flex items-center">
                ...
                <select
                  className={`base-input flex-1 ${editMode ? '' : 'bg-gray-100 bg-opacity-60'}`}
                  disabled={!editMode}
                >
