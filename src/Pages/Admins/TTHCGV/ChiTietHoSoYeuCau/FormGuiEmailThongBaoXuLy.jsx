...
import { putHoSoThuTucGuiYeuCauById } from '@/Apis/ApiGiangVien/ThuTucHanhChinhGiangVien/apiThuTucHanhChinhGiangVien'
import { getTrangThaiIDBySTTYeuCauId } from '@/Apis/ApiGiangVien/ThuTucHanhChinhGiangVien/apiTrangThai'
import {
...
const FormGuiEmailThongBaoXuLy = (props) => {
  ...
  const handleTiepNhanHoSo = () => {
    if (
      !isDTPheDuyet &&
      dataCBGV.HT_GROUPUSER_ID.includes(ROLE_VIEW_ACTION_TTHCGV.CBNV_TTHCGV) ===
        false &&
      dataCBGV.HT_GROUPUSER_ID.includes(QTHT_UNETI_ONLINE) === false
    ) {
      return Swal.fire({
        icon: 'error',
        title: 'Lỗi',
  ...
  const handleThongBaoXuLyHoSo = async () => {
    ...
    if (currentStatusId !== 0) {
      ...
      if (
          ...
          if (result.isConfirmed) {
            ...
            if (resNewTrangThaiID.status === 200) {
              ...
              if (resPutHoSoThuTuc.status === 200) {
                Swal.fire({
                  title: 'Thông báo',
                  text: `Đã  hoàn thành bước ${infoStatus.MC_TTHC_GV_TrangThai_TenTrangThai} sơ! Tiếp tục xử lý yêu cầu!`,
                  icon: 'success',
                })
        ...
        if (
          dataCBGV.HT_GROUPUSER_ID?.includes('24') === false &&
          dataCBGV.HT_GROUPUSER_ID?.includes('1') === false
        ) {
          return Swal.fire({
            icon: 'error',
            title: 'Lỗi',
            text: 'Thầy/Cô không có quyền được xử lý bước này!',
          })
        }
        ...
        if (+isTPPheDuyet === 0) {
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                ...
                if (resPutHoSoThuTuc.status === 200) {
                  ...
                  console.log('- isSendEmailCBNV: ', isSendEmailCBNV)
                  if (isSendEmailCBNV === true) {
                    console.log('send email CBNV')
                    try {
                      // eslint-disable-next-line no-unused-vars
                      const response = listDataCBNVPhanQuyen?.map(
                        async (cbnv) => {
                    ...
                    console.log("Don't send email to CBNV!")
                  }
        ...
        } else if (+isTPPheDuyet === 3) {
          //   TH2.4: BGH đã phê duyệt
          const resNewTrangThaiID = await getTrangThaiIDBySTTYeuCauId(
            dataDetailYeuCau.MC_TTHC_GV_GuiYeuCau_YeuCau_ID,
            dataDetailYeuCau.MC_TTHC_GV_TrangThai_STT + 2,
          )
          ...
          if (resNewTrangThaiID.status === 200) {
            ...
            Swal.fire({
              icon: 'question',
              title:
                'Xác nhận Ban Giám Hiệu đã phê duyệt bước này và chuyển đến bước trả kết quả?',
              html: `
                    <ul style='display: flex; flex-direction: column; align-items: flex-start'>
                        ${contentEmail.trim() === '' ? '<li>- Nội dung thông báo email không có.</li>' : ''}
                        ${linkAttachedFile.trim() === '' ? '<li>- Link tệp đính kèm không có.</li>' : ''}
              ...
              if (result.isConfirmed) {
                ...
                if (resPutHoSoThuTuc.status === 200) {
                  ...
                  if (isSendEmailCBNV == true) {
                    try {
                      listDataCBNVPhanQuyen.map(async (cbnv) => {
                        return await sendEmailTTHCGV_TP_CBNV(
                          listYeuCauPheDuyet[3].label,
                          {
                            ...dataDetailYeuCau,
                            ...newDataTPTrinhDuyetUpdate,
                          },
        ...
        if (
          dataCBGV.HT_GROUPUSER_ID?.includes('25') === false &&
          dataCBGV.HT_GROUPUSER_ID?.includes('1') === false
        ) {
          return Swal.fire({
            icon: 'error',
  ...
  const handleCancelHoSo = () => {
    const valuesRoleTTHCGV = Object.values(ROLE_VIEW_ACTION_TTHCGV)
    const containsValues = valuesRoleTTHCGV.some((value) =>
      dataCBGV.HT_GROUPUSER_ID.includes(value),
    )
    /**
     !isDTPheDuyet &&
    ...
    if (isTPPheDuyet === true) {
      if (isBGHPheDuyet === true) {
        if (isDoiTuongXuLy === 24) {
          return (
            <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-10 mb-6">
              <label
    ...
    <div className="my-4">
      <div className="form__content border border-gray-400 p-4 rounded-lg mb-4">
        ...
        <div className={clsx('mb-6')}>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-36 border p-2 rounded-md">
          ...
          <div className="ml-6">
            ...
            <div className="flex flex-col mb-2">
              ...
              <input
                ...
                accept=".pdf, .docx, .doc, .jpeg, .jpg, .png, .gif, .xls, .xlsx"
                value={''}
                onChange={async (e) => {
                  ...
                  if (
                    !file.name.match(/\.(pdf|docx|doc|jpeg|jpg|png|xls|xlsx)$/i)
                  ) {
                    Swal.fire({
                      icon: 'error',
                      title:
                        'Tệp tải lên không đúng định dạng yêu cầu. Vui lòng kiểm tra lại.',
                      text: 'Các loại file tải lên phải có dạng PDF, XLS, XLSX, DOC, DOCX, PNG, JPG, hoặc JPEG (Kích thước tối đa 5MB)',
                    })
                    return
                  } else {
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">
                ...
                <span className="font-medium">XLS</span> hoặc{' '}
      ...
      <div className="form__action flex gap-4 justify-center">
        ...
        <button
          ...
          className={clsx(
            'px-10 py-1 rounded-full font-medium whitespace-nowrap border border-red-500 focus:outline-red-500 bg-red-500 text-white shadow-sm hover:opacity-80 hover:shadow-xl',
