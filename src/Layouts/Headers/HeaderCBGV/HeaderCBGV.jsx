  ...
  return (
    <header
      className={transformCls([
        bem.b(),
        'fixed left-0 right-0 top-0 w-full bg-white',
      ])}
    >
      <nav className="">
        <div className="max-w-7xl flex flex-wrap items-center justify-between mx-auto relative">
          ...
          <Link to="/uneti" className="flex items-center p-2">
            <img src={logoUNETI} className="h-12 mr-3" alt="UNETI Logo" />
          </Link>
          {/* END: Logo */}

          {/* START: USER - Profile */}
          <div
            className="flex items-center md:order-2 relative py-2"
            id="control-dropdown"
          >
            <button
              type="button"
            ...
            {/* Dropdown menu */}
            <div
              className={`min-w-[220px] z-50 absolute top-[75%] my-4 right-0 text-base list-none bg-white divide-y divide-gray-100 rounded-b-lg shadow`}
              id="user-dropdown"
            >
          ...
          {/* START: Navbar Pages */}
          {(pathname === '/ho-tro-thiet-bi-phan-mem' ||
            pathname.includes('/ho-tro-thiet-bi-phan-mem/')) && (
          ...
          {pathname.includes('/huong-dan-su-dung-phan-mem') && (
            <div className="hidden lg:block col-span-8 lg:col-span-7">
              <p className="w-full text-center text-2xl font-bold uppercase text-uneti-primary">
          ...
          {pathname === '/uneti' && (
            <div className="hidden lg:block col-span-7 flex-1">
              <div className="inline-block py-1 border-b-4 border-sky-600 text-left text-md">
                <p className="w-full text-left text-md lg:text-xl font-bold uppercase text-uneti-primary">
                  Trường Đại Học Kinh Tế - Kỹ Thuật Công Nghiệp
                </p>
                <p>University of Economics - Technology for Industries</p>
              </div>
              <p className="hidden lg:inline-block uppercase text-md xl:text-xl font-bold my-2 text-red-600">
