...
export default function DrawerKetQuaDanhGiaTieuChi({
  isOpen,
  setIsOpen,
  form,
  setForm,
    ...
    <Drawer ref={drawerRef}>
      ...
      <DrawerContent>
        <div className="flex flex-col gap-3">
          ...
          <div>
            <p className="text-vs-text ml-2">Góp ý</p>
            <input
              value={form.KT_KDCL_DGDC_NghienCuu_BaoCao_GopY || ''}
              onChange={(event) =>
                setForm({
                  ...form,
                  KT_KDCL_DGDC_NghienCuu_BaoCao_GopY: event.target.value,
                })
              }
              type="number"
              className="base-input w-full"
              placeholder="Nhập góp ý (nếu có)"
            />
          </div>
        </div>
      </DrawerContent>
