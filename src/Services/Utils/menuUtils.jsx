...
export const getMenuWithSearchParams = (menus = [], menuIndex = 0) => {
  const [activeMenuIndex, setActiveMenuIndex] = useState(0)
  const navigate = useNavigate()
  ...
  if (menus.length > 0) {
    return (
      <ul className="flex flex-col gap-2">
        {menus.map(
          ({ path, label, children, sidebarActive, searchParams }, index) => {
            const isActive = index === activeMenuIndex
            if (!sidebarActive) {
              Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Chức năng đang được phát triển',
              })
              return
            }
            ...
            return (
              <li key={index}>
                <div
                  ...
                  to={{ pathname: path, search: searchParams }}
                  className={clsx(
                    'text-md cursor-pointer p-2 font-semibold hover:bg-uneti-primary hover:text-white',
                    menuIndex == 0 && 'rounded-lg',
                  )}
